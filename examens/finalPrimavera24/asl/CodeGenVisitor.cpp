...
std::any CodeGenVisitor::visitAssignStmt(AslParser::AssignStmtContext *ctx) {
  ...
  TypesMgr::TypeId tLeftExpr = getTypeDecor(ctx->left_expr());
  TypesMgr::TypeId tExpr = getTypeDecor(ctx->expr());
  std::string addr2_c = "%" + codeCounters.newTEMP();

  // Abans d'assignar el valor el convertim a float si cal
  if (Types.isFloatTy(tLeftExpr) and Types.isIntegerTy(tExpr)) {
  ...
  if (offs1 != "") {
    ...
    // Si tenim dos arrays l'hem de carregar tota
    if (Types.isArrayTy(tLeftExpr) and Types.isArrayTy(tExpr)) {
      size_t size = Types.getArraySize(tLeftExpr);
...
std::any CodeGenVisitor::visitSwapStmt(AslParser::SwapStmtContext *ctx) {
  DEBUG_ENTER();
  instructionList code;
  CodeAttribs     && codAtsLeft = std::any_cast<CodeAttribs>(visit(ctx->left_expr(0)));
  std::string          addrLeft = codAtsLeft.addr;
  std::string          offsLeft = codAtsLeft.offs;
  instructionList &    codeLeft = codAtsLeft.code;

  CodeAttribs     && codAtsRight = std::any_cast<CodeAttribs>(visit(ctx->left_expr(1)));
  std::string          addrRight = codAtsRight.addr;
  std::string          offsRight = codAtsRight.offs;
  instructionList &    codeRight = codAtsRight.code;
  code = codeLeft || codeRight;

  TypesMgr::TypeId tLeft = getTypeDecor(ctx->left_expr(0));
  TypesMgr::TypeId tRight = getTypeDecor(ctx->left_expr(0));
  // Assignem el temporal a l'adreça del segon
  std::string tempX = "%" + codeCounters.newTEMP();
  std::string tempY = "%" + codeCounters.newTEMP();
  if (Types.isArrayTy(tLeft) and Types.isArrayTy(tRight)) {
    // Hem de canviar tota l'array
    size_t size = Types.getArraySize(tLeft);
    std::string tIdx = "%" + codeCounters.newTEMP();
    for (size_t i = 0; i < size; ++i) {
      code = code || instruction::ILOAD(tIdx, std::to_string(i));
      // tempX = x[i]
      code = code || instruction::LOADX(tempX, addrLeft, tIdx);
      // tempY = y[i]
      code = code || instruction::LOADX(tempY, addrRight, tIdx);
      // x[i] = tempY
      code = code || instruction::XLOAD(addrLeft, tIdx, tempY);
      // y[i] = tempX
      code = code || instruction::XLOAD(addrRight, tIdx, tempX);
    }
  } else {
    // swap (x,y)
    // tempX = x[offsx]
    if (offsLeft == "") code = code || instruction::LOAD(tempX, addrLeft);
    else code = code || instruction::LOADX(tempX, addrLeft, offsLeft);
    
    // tempY = y[offsy]
    if (offsRight == "") tempY = addrRight;
    else code = code || instruction::LOADX(tempY, addrRight, offsRight);
    
    // x[offsx] = tempY
    if (offsLeft == "") code = code || instruction::LOAD(addrLeft, tempY);
    else code = code || instruction::XLOAD(addrLeft, offsLeft, tempY);
    
    // y[offsy] = tempX
    if (offsRight == "") code = code || instruction::LOAD(addrRight, tempX);
    else code = code || instruction::XLOAD(addrRight, offsRight, tempX);
  }
