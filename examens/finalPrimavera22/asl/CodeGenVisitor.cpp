...
std::any CodeGenVisitor::visitFactorialExpr(AslParser::FactorialExprContext *ctx) {
  DEBUG_ENTER();
  instructionList code;
  ...
  std::string label = codeCounters.newLabelWHILE();
  std::string baseCase = "baseCase"+label;
  std::string inductiveCase = "inductiveCase"+label;
  std::string endLabel = "endLabel"+label;
  
  std::string jumpCondition = "%" + codeCounters.newTEMP();
  std::string i = "%" + codeCounters.newTEMP();
  std::string tZero = "%" + codeCounters.newTEMP();
  std::string tOne = "%" + codeCounters.newTEMP();
  std::string tTwo = "%" + codeCounters.newTEMP();
  std::string result = "%" + codeCounters.newTEMP();

  code = code || instruction::ILOAD(tZero, "0");
  code = code || instruction::ILOAD(tOne, "1");
  code = code || instruction::ILOAD(tTwo, "2");
  code = code || instruction::ILOAD(result, "1");
  code = code || instruction::ILOAD(i, "2");
  
  // jumpCondition = addrExpr < 0
  code = code || instruction::LT(jumpCondition, addrExpr, tZero);
  code = code || instruction::FJUMP(jumpCondition,baseCase);

  // Mes petit que zero 
  code = code || instruction::HALT(code::INVALID_INTEGER_OPERAND);
  code = code || instruction::UJUMP(endLabel);

  // jumpCondition = addrExpr < 2
  code = code || instruction::LABEL(baseCase);
  code = code || instruction::LT(jumpCondition, addrExpr, tTwo);
  code = code || instruction::FJUMP(jumpCondition, inductiveCase);
  ...
  // result = 1
  // i = 2
  code = code || instruction::LABEL(inductiveCase);
  // While i <= addrExpr, { result = result *i; i=i+1 }
  code = code || instruction::LE(jumpCondition, i, addrExpr);
  code = code || instruction::FJUMP(jumpCondition, endLabel);
  // Interior bucle
  code = code || instruction::MUL(result,result, i);
  code = code || instruction::ADD(i,i,tOne);
  code = code || instruction::UJUMP(inductiveCase);
