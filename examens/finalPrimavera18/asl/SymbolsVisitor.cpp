...
std::any SymbolsVisitor::visitPairType(AslParser::PairTypeContext *ctx) {
  DEBUG_ENTER();
  visit(ctx->basicType(0));
  visit(ctx->basicType(1));
  TypesMgr::TypeId tFirst = getTypeDecor(ctx->basicType(0));
  TypesMgr::TypeId tSecond = getTypeDecor(ctx->basicType(1));
  TypesMgr::TypeId tRet = Types.createPairTy(tFirst, tSecond);
  putTypeDecor(ctx, tRet);
  DEBUG_EXIT();
  return 0;
}
