...
std::any TypeCheckVisitor::visitSumExpr(AslParser::SumExprContext *ctx) {
    DEBUG_ENTER();
    visitChildren(ctx);
    int size = ctx->expr().size();
    bool error = false;
    bool someFloat = false;
    for (int i = 0; i < size and not error; ++i) {
    ...
    if (not error and someFloat) putTypeDecor(ctx, Types.createFloatTy());
