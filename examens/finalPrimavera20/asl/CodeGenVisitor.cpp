...
std::any CodeGenVisitor::visitSumExpr(AslParser::SumExprContext *ctx) {
  DEBUG_ENTER();
  instructionList code;
  TypesMgr::TypeId tSum = getTypeDecor(ctx);
  int size = ctx->expr().size();
  std::string result = "%" + codeCounters.newTEMP();
  if (Types.isFloatTy(tSum)) {
    code = code || instruction::FLOAD(result, "0.0");
    for (int i = 0; i < size; ++i) {
      ...
      code = code || codeExpr;
      // Conversion si cal
      std::string addrExpr_f = addrExpr;
      if (Types.isIntegerTy(getTypeDecor(ctx->expr(i)))) {
          addrExpr_f = "%" + codeCounters.newTEMP();
          code = code || instruction::FLOAT(addrExpr_f, addrExpr);
      }
      code = code || instruction::FADD(result, result, addrExpr_f);
    }
  } 
  else {
    ...
    for (int i = 0; i < size; ++i) {
      ...
      instructionList & codeExpr = codAtsExpr.code;
      code = code || codeExpr;
      code = code || instruction::ADD(result, result, addrExpr);
    }
  } 
  // Retornem només addr=nom_array, offs=index
  CodeAttribs codAts(result, "", code);
