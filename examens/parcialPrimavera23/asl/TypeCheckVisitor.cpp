...
std::any TypeCheckVisitor::visitCaseStmt(AslParser::CaseStmtContext *ctx) {
    ...
    if (not (Types.isIntegerTy(t) or Types.isCharacterTy(t))) {
        Errors.incompatibleExpressionInCase(ctx);
        putTypeDecor(ctx->expr(), Types.createErrorTy());
    }
...
std::any TypeCheckVisitor::visitNullToken(AslParser::NullTokenContext *ctx) {
    DEBUG_ENTER();
    putTypeDecor(ctx, Types.createPointerTy(Types.createVoidTy()));
    DEBUG_EXIT();
    return 0;
}


std::any TypeCheckVisitor::visitPointerCreation(AslParser::PointerCreationContext *ctx) {
    DEBUG_ENTER();
    visit(ctx->expr());
    TypesMgr::TypeId t = getTypeDecor(ctx->expr());
    putTypeDecor(ctx, Types.createPointerTy(t));
    DEBUG_EXIT();
    return 0;
}

std::any TypeCheckVisitor::visitPointerDereferentation(AslParser::PointerDereferentationContext *ctx) {
    DEBUG_ENTER();
    visit(ctx->expr());
    TypesMgr::TypeId t = getTypeDecor(ctx->expr());
    if (not Types.isPointerTy(t)) {
        Errors.nonPointerInPointerAccess(ctx);
    } else {
        putTypeDecor(ctx, Types.getPointedType(t));
    }
