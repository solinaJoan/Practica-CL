...
std::any TypeCheckVisitor::visitThrow(AslParser::ThrowContext *ctx){
    ...
    visit(ctx->expr());
    TypesMgr::TypeId t = getTypeDecor(ctx->expr());
    if (not Types.isPrimitiveTy(t) and not Types.isErrorTy(t))
...
std::any TypeCheckVisitor::visitTryCatch(AslParser::TryCatchContext *ctx){
    ...
    std::vector<TypesMgr::TypeId> typesCases;
    for (auto lcasesCtx : ctx->lcases()) {
        visit(lcasesCtx);
        TypesMgr::TypeId t = getTypeDecor(lcasesCtx->expr());
    ...
    // if (ctx->lcases().size() > 1) {
    //     visit(ctx->lcases(0)->expr());
    //     TypesMgr::TypeId t = getTypeDecor(ctx->lcases(0)->expr());

    //     for (auto lcasesCtx : ctx->lcases()) {
    //         visit(lcasesCtx);
    //         TypesMgr::TypeId t1 = getTypeDecor(lcasesCtx->expr());
    //         if  (not Types.isErrorTy(t1) and not Types.isPrimitiveTy(t1)) {
    //             Errors.catchCasesRequireBasicTypes(ctx->CATCH());
    //         }
    //         // Aquest salta si el primer és correcte
    //         else if (Types.isPrimitiveTy(t1) and Types.isPrimitiveTy(t) and not Types.copyableTypes(t, t1)) {
    //             Errors.catchCasesRequireCompatibleTypes(ctx->CATCH());
    //         } 
    //     }
    // }
    // else if (ctx->lcases().size() == 1) {
    //     visit(ctx->lcases(0)->expr());
    //     TypesMgr::TypeId t = getTypeDecor(ctx->lcases(0)->expr());
...
std::any TypeCheckVisitor::visitArrayConstructor(AslParser::ArrayConstructorContext *ctx) {
    DEBUG_ENTER();
    std::vector<TypesMgr::TypeId> typesCases;
