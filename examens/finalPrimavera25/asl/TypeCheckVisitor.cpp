...
std::any TypeCheckVisitor::visitArray(AslParser::ArrayContext *ctx)
{
    ...
    if (ctx->expr().size() == 1) {
        ...
        if (not Types.isErrorTy(tExpr2) and Types.isArrayTy(tArray) and not Types.copyableTypes(Types.getArrayElemType(tArray), tExpr2)) {
            Errors.inCompatibleSecureAccessExpression(ctx->expr(1));
        }
    }
    DEBUG_EXIT();
    return 0;
...
std::any TypeCheckVisitor::visitZipStmt(AslParser::ZipStmtContext *ctx){
    DEBUG_ENTER();
    visit(ctx->expr(0));
    visit(ctx->expr(1));
    visit(ctx->expr(2));
    TypesMgr::TypeId A = getTypeDecor(ctx->expr(0));
    TypesMgr::TypeId B = getTypeDecor(ctx->expr(1));
    TypesMgr::TypeId C = getTypeDecor(ctx->expr(2));
    if (not Types.isErrorTy(A) and not Types.isErrorTy(B) and not Types.isErrorTy(C)) {
        if (not Types.isArrayTy(A) or not Types.isArrayTy(B) or not Types.isArrayTy(C)) {
            Errors.incompatibleArgumentsInZip(ctx);
        } else {
            int sizeA = Types.getArraySize(A);
            int sizeB = Types.getArraySize(B);
            int sizeC = Types.getArraySize(C);
            TypesMgr::TypeId elemA = Types.getArrayElemType(A);
            TypesMgr::TypeId elemB = Types.getArrayElemType(B);
            
            if ((not Types.copyableTypes(elemA, elemB) and not Types.copyableTypes(elemB, elemA)) or 2*std::min(sizeA, sizeB) != sizeC) {
                Errors.incompatibleArgumentsInZip(ctx);
            }
        }
    }
