...
std::any CodeGenVisitor::visitArray(AslParser::ArrayContext *ctx) {
  ...
  if (ctx->expr().size() == 1) {
    ...
    int sizeArray = Types.getArraySize(getTypeDecor(ctx->ident()));

    // Temporals 
    std::string tZero    = "%" + codeCounters.newTEMP();
    std::string tSize    = "%" + codeCounters.newTEMP();
    std::string tLow     = "%" + codeCounters.newTEMP();  // index >= 0
    std::string tHigh    = "%" + codeCounters.newTEMP();  // index < mida
    std::string tInRange = "%" + codeCounters.newTEMP();  // tLow AND tHigh
    std::string tResult  = "%" + codeCounters.newTEMP();  // resultat final

    std::string label = codeCounters.newLabelIF();
    std::string labelOOB = "labelOOB" + label;
    std::string labelEnd = "labelEnd" + label;  

    code = code || instruction::ILOAD(tZero, "0");
    code = code || instruction::LE(tLow, tZero, addrExpr);
    code = code || instruction::ILOAD(tSize, std::to_string(sizeArray));
    code = code || instruction::LT(tHigh, addrExpr, tSize);
    code = code || instruction::AND(tInRange, tLow, tHigh);
    code = code || instruction::FJUMP(tInRange, labelOOB);
    // Dins de rang
    code = code || instruction::LOADX(tResult, addrIdent, addrExpr);
    code = code || instruction::UJUMP(labelEnd);
    // Fora de rang
    code = code || instruction::LABEL(labelOOB);
    
    if (Types.isFloatTy(Types.getArrayElemType(getTypeDecor(ctx->ident()))) and Types.isIntegerTy(getTypeDecor(ctx->expr(1)))) {
      code = code || instruction::FLOAT(tResult, addrExpr2);
    } else {
      code = code || instruction::LOAD(tResult, addrExpr2);
    }
    code = code || instruction::LABEL(labelEnd);

    // Retornem tResult com a variable amb el valor (offs buit = ja resolt)
    CodeAttribs codAts(tResult, "", code);
    return codAts;
  }
  DEBUG_EXIT();
}

std::any CodeGenVisitor::visitZipStmt(AslParser::ZipStmtContext *ctx) {
  DEBUG_ENTER();
  instructionList code;
