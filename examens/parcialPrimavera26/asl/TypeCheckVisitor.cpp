...
std::any TypeCheckVisitor::visitArithmetic(AslParser::ArithmeticContext *ctx)
{
    ...
    if (ctx->op->getText() == "%") {
        ...
        if (ctx->op->getText() == "+" and (isStringOrChar(t1) or isStringOrChar(t2))) {
            if (not Types.isErrorTy(t1) and not Types.isErrorTy(t2) and 
                    not (isStringOrChar(t1) and isStringOrChar(t2))) {
                Errors.incompatibleOperator(ctx->op);
            }
            tRes = Types.createStringTy();
        } else {
...
std::any TypeCheckVisitor::visitStringVal(AslParser::StringValContext *ctx)
{
    DEBUG_ENTER();
    TypesMgr::TypeId t = Types.createStringTy();
    putTypeDecor(ctx, t);
    putIsLValueDecor(ctx, false);
    DEBUG_EXIT();
    return 0;
...
std::any TypeCheckVisitor::visitArray(AslParser::ArrayContext *ctx)
{
    ...
    if (not Types.isMapTy(tArray)) {
        ...
        if (not Types.isErrorTy(tExpr) and not Types.copyableTypes(Types.getMapKeyType(tArray), tExpr)) {
            Errors.invalidKeyTypeInMapAccess(ctx->expr());
        }
        putTypeDecor(ctx,Types.getMapValueType(tArray));
    }
...
bool TypeCheckVisitor::isStringOrChar(TypesMgr::TypeId type)
{
    return Types.isCharacterTy(type) or Types.isStringTy(type);
}

std::any TypeCheckVisitor::visitIndexExpr(AslParser::IndexExprContext *ctx) {
    DEBUG_ENTER();
    visit(ctx->expr(0));
    visit(ctx->expr(1));
    ...
    if (not Types.isErrorTy(t0) and not Types.isErrorTy(t1) and 
            (not isStringOrChar(t0) or not Types.isStringTy(t1))) {
        Errors.incompatibleOperator(ctx->op);
    }
...
std::any TypeCheckVisitor::visitLenghtExpr(AslParser::LenghtExprContext *ctx) {
    DEBUG_ENTER();
    visit(ctx->expr());
    TypesMgr::TypeId t0 = getTypeDecor(ctx->expr());
    if (not Types.isErrorTy(t0) and not Types.isStringTy(t0)) {
            Errors.incompatibleOperator(ctx->op);
    }
    putTypeDecor(ctx, Types.createIntegerTy());
    DEBUG_EXIT();
...
std::any TypeCheckVisitor::visitInExpr(AslParser::InExprContext *ctx) {
    DEBUG_ENTER();
    visit(ctx->expr(0));
    visit(ctx->expr(1));
    TypesMgr::TypeId t0 = getTypeDecor(ctx->expr(0));
...
std::any TypeCheckVisitor::visitMapIterationStmt(AslParser::MapIterationStmtContext *ctx) {
    DEBUG_ENTER();
    visitChildren(ctx);
    TypesMgr::TypeId tkey = getTypeDecor(ctx->expr(0));
    TypesMgr::TypeId tValue = getTypeDecor(ctx->expr(1));
    TypesMgr::TypeId tMap = getTypeDecor(ctx->expr(2));
    if (not Types.isErrorTy(tMap)) {
        if (not Types.isMapTy(tMap)) {
            Errors.forMapStmtAppliedToNonMapVar(ctx->expr(2));
        } else {
            if (not Types.isErrorTy(tkey) and not Types.equalTypes(Types.getMapKeyType(tMap),tkey)) {
                Errors.forMapStmtIncompatibleKeyVar(ctx->expr(0));
            } 
            if (not Types.isErrorTy(tValue) and not Types.equalTypes(Types.getMapValueType(tMap), tValue)) {
                Errors.forMapStmtIncompatibleValueVar(ctx->expr(1));
            }
        } 
    }
    DEBUG_EXIT();
