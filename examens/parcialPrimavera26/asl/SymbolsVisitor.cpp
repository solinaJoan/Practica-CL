...
std::any SymbolsVisitor::visitBasicType(AslParser::BasicTypeContext *ctx) {
  ...
  else if (ctx->STRINGTYPE()) t = Types.createStringTy();
  putTypeDecor(ctx, t);
  DEBUG_EXIT();
  return 0;
}
...
std::any SymbolsVisitor::visitMapType(AslParser::MapTypeContext *ctx) {
  ...
  TypesMgr::TypeId tKey = getTypeDecor(ctx->basicType(0));
  TypesMgr::TypeId tValue = getTypeDecor(ctx->basicType(1));
  TypesMgr::TypeId tRet = Types.createMapTy(tKey,tValue);
  putTypeDecor(ctx, tRet);
  DEBUG_EXIT();
  return 0;
}
