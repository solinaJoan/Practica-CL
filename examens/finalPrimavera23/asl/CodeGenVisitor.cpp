...
std::any CodeGenVisitor::visitReduceExpr(AslParser::ReduceExprContext *ctx) {
  DEBUG_ENTER();
  instructionList code;
  CodeAttribs && codAtsArray = std::any_cast<CodeAttribs>(visit(ctx->expr(0)));
  std::string       addrArray = codAtsArray.addr;
  instructionList & codeArray = codAtsArray.code;
  CodeAttribs && codAtsFunc = std::any_cast<CodeAttribs>(visit(ctx->expr(1)));
  std::string       addrFunc = codAtsFunc.addr;
  instructionList & codeFunc = codAtsFunc.code;
  code = codeArray || codeFunc;
  TypesMgr::TypeId tArray = getTypeDecor(ctx->expr(0));
  int size = Types.getArraySize(tArray);
  std::string name = ctx->expr(1)->getText();

  // Si l'array té mida 1 descartem el codi de la funció i retornem A[0]
  if (size < 2) {
    CodeAttribs codAts(addrArray, "", codeArray);
    return codAts;
  }

  std::string idx = "%" + codeCounters.newTEMP();
  std::string arrayElem = "%" + codeCounters.newTEMP();
  std::string accum = "%" + codeCounters.newTEMP();
  code = code || instruction::LOAD(accum, addrArray);

  // Si no, fem tota la crida reduce
  for (int i = 1; i < size; ++i) {
    code = code || instruction::ILOAD(idx, std::to_string(i));
    code = code || instruction::LOADX(arrayElem, addrArray, idx);
    // Cridem la funcio amb l'acumulador i el valor carregat de l'array
    code = code || instruction::PUSH();
    code = code || instruction::PUSH(accum);
    code = code || instruction::PUSH(arrayElem);
    code = code || instruction::CALL(name);
    code = code || instruction::POP();
    code = code || instruction::POP();
    code = code || instruction::POP(accum);
  }

  CodeAttribs codAts(accum, "", code);
  DEBUG_EXIT();
  return codAts;
}
