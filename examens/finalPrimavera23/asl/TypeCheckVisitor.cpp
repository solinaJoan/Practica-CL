...
std::any TypeCheckVisitor::visitReduceExpr(AslParser::ReduceExprContext *ctx) {
    DEBUG_ENTER();
    visit(ctx->expr(0));
    visit(ctx->expr(1));
    TypesMgr::TypeId tArray = getTypeDecor(ctx->expr(0));
    TypesMgr::TypeId tFunc = getTypeDecor(ctx->expr(1));

    if (not Types.isArrayTy(tArray)) {
        Errors.arrayIsRequired(ctx);
    } else if (not Types.isFunctionTy(tFunc) or not Types.reduceValidFunction(tFunc)) {
        Errors.reduceInvalidFunction(ctx);
    } else if (not Types.copyableTypes(Types.getFuncReturnType(tFunc), Types.getArrayElemType(tArray))) {
        Errors.reduceIncompatibleArguments(ctx);
    } else {
        putTypeDecor(ctx, Types.getFuncReturnType(tFunc));
        putIsLValueDecor(ctx, false);
    }
    DEBUG_EXIT();
    return 0;
