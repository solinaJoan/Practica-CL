...
std::any TypeCheckVisitor::visitPackStmt(AslParser::PackStmtContext *ctx) {
    DEBUG_ENTER();
    if (ctx->PACK()) {
        visitPackStmtPrivate(ctx,true);
    } else {
        visitPackStmtPrivate(ctx,false);
    }
    DEBUG_EXIT();
    return 0;
}

std::any TypeCheckVisitor::visitPackStmtPrivate(AslParser::PackStmtContext *ctx, bool pack) {
    DEBUG_ENTER();
    visitChildren(ctx);
    TypesMgr::TypeId t = getTypeDecor(ctx->ident());
    ...
    if (not Types.isErrorTy(t)) {
        if (not Types.isArrayTy(t)) {
            ...
            int sizeArray = Types.getArraySize(t);
            int sizeExpr = ctx->expr().size();
            if (sizeExpr != sizeArray) {
                Errors.packUnpackNumberOfItemsMismatch(ctx);
            }
            TypesMgr::TypeId tElem = Types.getArrayElemType(t);

            int limit;
            if (pack) limit = sizeExpr;
            else limit = sizeArray;

            for (int i=0; i<limit; ++i) {
                TypesMgr::TypeId tExpr = getTypeDecor(ctx->expr(i));
                if (not pack) {
                    TypesMgr::TypeId taux = tExpr;
                    tExpr = tElem;
                    tElem = taux;
                }

                if (not Types.isErrorTy(tExpr) and not Types.isErrorTy(tElem) and not Types.copyableTypes(tElem,tExpr)) {
