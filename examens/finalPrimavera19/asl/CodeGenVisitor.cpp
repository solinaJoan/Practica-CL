...
std::any CodeGenVisitor::visitArray(AslParser::ArrayContext *ctx) {
  ...
  instructionList && error = throwRuntimeErrorTCode(ctx);

  code = code || instruction::ILOAD(tArraySize, std::to_string(arraySize));
  code = code || instruction::ILOAD(tZero, "0");
  // LT_size = addrIdx < arraySize
...
std::any CodeGenVisitor::visitMaxExpr(AslParser::MaxExprContext *ctx) {
  DEBUG_ENTER();
  instructionList code;
  ...
  TypesMgr::TypeId tArray = getTypeDecor(ctx->expr());
  bool isFloat = Types.isFloatTy(Types.getArrayElemType(tArray));
  int size = Types.getArraySize(tArray);
  std::string temp = "%" + codeCounters.newTEMP();
  std::string jumpCondition = "%" + codeCounters.newTEMP();
  std::string tMax = "%" + codeCounters.newTEMP();
  std::string idx = "%" + codeCounters.newTEMP();
  code = code || instruction::LOAD(tMax, addrE);

  for (int i = 0; i < size; ++i) {
    code = code || instruction::ILOAD(idx, std::to_string(i));
    code = code || instruction::LOADX(temp, addrE, idx);
    // if max < A[i] then max = A[i]
    std::string label = codeCounters.newLabelIF();
    std::string labelEndIf = "endif"+label;
    if (isFloat) code = code || instruction::FLT(jumpCondition, tMax, temp);
    else code = code || instruction::LT(jumpCondition, tMax, temp);
    code = code || instruction::FJUMP(jumpCondition, labelEndIf);
    code = code || instruction::LOAD(tMax, temp);
    code = code || instruction::LABEL(labelEndIf);
  }
  CodeAttribs codAts(tMax, "", code);
  DEBUG_EXIT();
  return codAts;
}
