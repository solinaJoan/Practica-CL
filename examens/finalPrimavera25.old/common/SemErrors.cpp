...
void SemErrors::inCompatibleSecureAccessExpression(antlr4::ParserRuleContext *ctx) {
  ErrorInfo error(ctx->getStart()->getLine(), ctx->getStart()->getCharPositionInLine(), "Secure array access with incompatible expression.");
  ErrorList.push_back(error);
}

void SemErrors::incompatibleArgumentsInZip(antlr4::ParserRuleContext *ctx) {
  ErrorInfo error(ctx->getStart()->getLine(), ctx->getStart()->getCharPositionInLine(), "Zip instruction with inappropriate or incompatible arguments.");
  ErrorList.push_back(error);
}
