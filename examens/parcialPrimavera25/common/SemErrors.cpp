...
void SemErrors::anyCastFromNonAny(antlr4::ParserRuleContext *ctx) {
  ErrorInfo error(ctx->getStart()->getLine(), ctx->getStart()->getCharPositionInLine(), "`any-cast' requires an expression of type `any`.");
  ErrorList.push_back(error);
}

void SemErrors::nonArrayInArraySlice(antlr4::ParserRuleContext *ctx) {
  ErrorInfo error(ctx->getStart()->getLine(), ctx->getStart()->getCharPositionInLine(), "Array slice in a non array operand.");
  ErrorList.push_back(error);
}

void SemErrors::invalidLimitsInSlice(antlr4::ParserRuleContext *ctx) {
  ErrorInfo error(ctx->getStart()->getLine(), ctx->getStart()->getCharPositionInLine(), "Array slice with invalid limits.");
  ErrorList.push_back(error);
}
