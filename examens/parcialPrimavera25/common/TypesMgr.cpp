...
TypesMgr::TypesMgr() {
  ...
  TypesVec[AnyTyId]       = Type(TypeKind::AnyKind);
}
...
TypesMgr::TypeId TypesMgr::createAnyTy() {
  return AnyTyId;
}
...
bool TypesMgr::isAnyTy(TypeId tid) const {
  return tid == AnyTyId;
}
...
bool TypesMgr::equalTypes(TypeId tid1, TypeId tid2) const {
  if (isAnyTy(tid1) or isAnyTy(tid2))
    return false;
  if (tid1 == tid2)
    return true;
...
bool TypesMgr::copyableTypes(TypeId tid1, TypeId tid2) const {
  if (isAnyTy(tid1)) return isPrimitiveNonVoidTy(tid2);
...
bool TypesMgr::Type::isAnyTy() const {
  return ID == TypeKind::AnyKind;
}
